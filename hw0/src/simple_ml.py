...
def softmax_loss(Z, y):
    ...
    """
    # BEGIN YOUR CODE
    loss = np.log(np.sum(np.exp(Z), axis=1)) - Z[np.arange(y.shape[0]), y]
    return np.mean(loss)
    # END YOUR CODE
...
def softmax_regression_epoch(X, y, theta, lr=0.1, batch=100):
    ...
    for i in range(iter_num):
        ...
        Z = X_batch @ theta
        grad = np.exp(Z) / np.sum(np.exp(Z), axis=1, keepdims=True)
        grad[np.arange(y_batch.shape[0]), y_batch] -= 1
        grad /= batch
        theta_grad = X_batch.T @ grad
        theta -= lr * theta_grad
